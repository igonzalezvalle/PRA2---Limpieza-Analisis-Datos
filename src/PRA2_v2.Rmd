...
Este modelo nos da una tasa de error del 27.87%. Comprobamos que la clasificación de vuelos retrasados no es buena. Comprobamos la matriz de confusión.
...
sort(table(vuelos_delay$ORIGIN_AIRPORT) / table(vuelos$ORIGIN_AIRPORT) * 100, decreasing = TRUE)

```


Vemos como el aeropuerto con mayor porcentaje de vuelos retrasado es el de GST con un 44.15% 
...
sort(table(vuelos_delay$DESTINATION_AIRPORT) / table(vuelos$DESTINATION_AIRPORT) * 100, decreasing = TRUE)
